...
| $p_{\theta}(x\mid z)$ | probabilistic decoder로, 주어진 $z$(latent vector)를 이용해 $x$(true data)를 
생성할 **likelihood**이다. |
...
- **VAE의 training 과정**
    ...
    5. 따라서, $\log p_{\theta}(x)$를 $\mathbb{E}_{q_{\phi}(z\mid x)}[\log p_{\theta}(x)]$로 나타낼 수 있다. 이 때, **bayes’ theorem**에 의해 이 식을 [아래]와 같이 변형할 수 있다. 결과적으로 두 개의 expected log-likelihood로 나타나는데, 각각 ELBO(Evidence Lower Bound)와 $q_{\phi}$와 $p_{\theta}$에 대한 KL-Divergence이다.
