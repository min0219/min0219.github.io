...
| $q_{\phi}(z|x)$ | **probabilistic encoder**로, estimated **posterior** probability function이다. |
| $p_{\theta}(x|z)$ | probabilistic decoder로, 주어진 $z$(latent vector)를 이용해 $x$(true data)를 
생성할 **likelihood**이다. |
...
- **VAE의 training 과정**
    ...
    5. 따라서, $\log p_{\theta}(x)$를 $\mathbb{E}_{q_{\phi}(z\mid x)}[\log p_{\theta}(x)]$로 나타낼 수 있다. 이 때, **bayes’ theorem**에 의해 이 식을 [아래](https://www.notion.so/Chapter4-Variational-Auto-Encoder-VAE-1ea2fa2bb9ec495480d29454df465610?pvs=21)와 같이 변형할 수 있다. 결과적으로 두 개의 expected log-likelihood로 나타나는데, 각각 ELBO(Evidence Lower Bound)와 $q_{\phi}$와 $p_{\theta}$에 대한 KL-Divergence이다.
    ...
    6. 이 때, ELBO에 대해 식을 정리하면 아래와 같은 식을 얻을 수 있는데, KL-Divergence는 항상 양수이므로 **ELBO는 log-likelihood $\log \,p_{\theta}(x)$의 lower bound**임을 알 수 있다. 따라서 **ELBO를 maximize함으로써 likelihood 또한 maximize**할 수 있다.
        ...
        ![Untitled 3](../images/2025-01-21-GM04/Untitled 3.png)
        
    7. ELBO를 변형하여 아래와 같이 두 개의 term으로 분리할 수 있다.
        - $-\mathbb{E}_{z \sim q_{\phi}(z\mid x)} \left [\log p_{\theta}(x\mid z) \right]$, `Reconstruction Error` : $q_{\phi}(z\mid x)$에서 sampling한 $z$에 대해 log-likelihood를 maximize하는 것이 목표이고, 이 때 Reconstruction Error를 minimize할 수 있다.
...
- Monte Carlo Sampling : 반복된 random sampling을 이용해 함수의 값을 근사하는 알고리즘이다.
    ...
    ![Untitled 3](../images/2025-01-21-GM04/Untitled 3-1737450083489-8.png)
...
- Entropy : [2.2-Latent variable model](https://www.notion.so/Chapter4-Variational-Auto-Encoder-VAE-1ea2fa2bb9ec495480d29454df465610?pvs=21)의 식에서는 log probability의 expectation의 음수를 뜻하며, **불확실성**을 의미한다.
